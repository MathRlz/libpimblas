...
uint64_t clever_dot_product(const uint32_t *arr1, const uint32_t *arr2) {
    uint64_t dp = 0;
    ...
    for (; exp >= PRECISION; --exp) {
        ...
        for (int j = exp - PRECISION + 1; j <= PRECISION - 1; ++j) {
            partSum += count_all_ones_32(arr1[j] & arr2[exp-j]);
        }
...
__dma_aligned uint32_t vec1[NR_TASKLETS][PRECISION];
__dma_aligned uint32_t vec2[NR_TASKLETS][PRECISION];
...
int main() {
  ...
  int blocks_per_tasklet_size = number_blocks_per_tasklet * PRECISION * sizeof(uint32_t);
  ...
  for (uint32_t block_id = 0; block_id < number_blocks_per_tasklet; block_id++) {
    ...
    mram_read((__mram_ptr void *)(vec1_MRAM + block_offset), vec1[tasklet_id], PRECISION * sizeof(uint32_t));
    mram_read((__mram_ptr void *)(vec2_MRAM + block_offset), vec2[tasklet_id], PRECISION * sizeof(uint32_t));
