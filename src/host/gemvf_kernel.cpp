...
void GEMVF_Kernel::set_y(const float *data, bool async) {
...
void GEMVF_Kernel::set_params(const float *alpha, const float *beta, bool async) {
  params args{.rows_per_dpu = this->rows_per_dpu, .row_size = n, .alpha = *alpha, .beta = *beta};
  this->set_arg_broadcast("args", 0, reinterpret_cast<uint8_t *>(&args), sizeof(params), async);
}
